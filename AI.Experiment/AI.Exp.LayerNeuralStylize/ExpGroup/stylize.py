...
STYLE_LAYERS = ('relu1_1', 'relu2_1', 'relu3_1', 'relu4_1', 'relu5_1')#在这里建立需要的层
...
#functools是一个函数工具，其中的functools.reduce与python内置的reduce相同，使代码能够兼容Python3
...
def stylize(network, initial, content, styles, iterations,
    ...
    """
    shape = (1,) + content.shape#把在shape前加上1，形成(1,x,y)
    style_shapes = [(1,) + style.shape for style in styles]
    content_features = {}
    style_features = [{} for _ in styles]

    # compute content features in feedforward mode
    g = tf.Graph()#Tensorflow是基于图（Graph）的计算系统
    with g.as_default(), g.device('/cpu:0'), tf.Session() as sess:
        image = tf.placeholder('float', shape=shape)#设置placeholder
        net, mean_pixel = vgg.net(network, image)
        content_pre = np.array([vgg.preprocess(content, mean_pixel)])
        content_features[CONTENT_LAYER] = net[CONTENT_LAYER].eval(
                feed_dict={image: content_pre})#获取特征数据
